...
- Persisted reconciliation queue and snapshot state so restart recovery keeps the pending event picture
...
- Restart recovery now preserves queued reconciliation events and the last reconciliation snapshot locally, but there is still no full exchange-grade replay source for missed remote events during downtime
...
- The reconciliation section of `/api/status` includes restored queued events and the last persisted reconciliation snapshot timestamp after restart
